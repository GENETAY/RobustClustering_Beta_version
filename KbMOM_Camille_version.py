...
'''
modified version to runs tests. the difference occurs line 172.
original verson was : sum(np.bincount(nearest_centroid) > 1) == self.K
'''
...
class KbMOM_CSBeta:
    ...
    def __init__(self,X,K,nbr_blocks,coef_ech = 6,max_iter = 10,outliers = None, confidence = 0.95, threshold = 0.001,quantile   = 0.5,initial_centers = None,init_type ='km++',averaging_strategy='cumul',nb_min_repr_by_cluster=2,random_state = 1):
        '''
    ...
    def init_centers_function(self,X,idx_blocks):
        ...
        if self.init_type=='km++':
            ...
            for idx_ in idx_blocks: 
                init_centers_ = kmedianpp_init(
                    X[idx_,:],
                    self.K,
                    x_squared_norms[idx_],
    ...
    def inertia_function(self,idx_block,centroids = None):
        ...
        if len(set(nearest_centroid)) == self.K and sum(np.bincount(nearest_centroid) >= self.nb_min_repr_by_cluster) == self.K :
